 ...
 <!-- Main Content -->
      ...
      <?php error_reporting(0); ?>
        ...
        <?php 
          ...
          foreach ($jmal as $key ) {
            $id=$key['id'];
            $nilai=query("SELECT b.id_kriteria,a.nilai_parameter
              FROM parameter a 
              JOIN nilai_analisis b USING(id_parameter)
              WHERE  b.id_kriteria=$id");
          ...
          foreach ($alt as $key) {
            $id=$key['id'];
            $nilai_normalisasi = query(" SELECT b.id_alternatif,a.nilai_parameter, a.id_kriteria
              FROM parameter a 
              JOIN nilai_analisis b USING(id_parameter)
              WHERE  b.id_alternatif=$id");
